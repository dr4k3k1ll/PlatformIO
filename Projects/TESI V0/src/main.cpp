...
void CameraConfigurazion(){
  ...
  if (err != ESP_OK) {
    Serial.printf("Inizializzazione Fallita, Errore 0x%x", err);
    return;
  }
...
void setup(){
  ...
  Serial.println("sto avviando la fotocamera...\n");

  CameraConfigurazion();

  Serial.println("Fotocamera Avviata! \n");
...
server.on("/camera", HTTP_GET, [](AsyncWebServerRequest *req){
    server.end();  // Arresta un server precedente prima di avviarne uno nuovo
   // AsyncWebServer server(80);  // Usa la porta 81
    startCameraServer();
    req->redirect("/");
});
server.begin(); // Avvia il server
