...
const char* ssid = MY_WIFI_SSID_2;
const char* password = MY_WIFI_PASS_2;
...
////FUNZIONI SERVER////
void MyDns(){ //Configurazione DNS
  if (MDNS.begin("DASHCAM32"))
  {
     Serial.println("\n connesso! \n");
    Serial.println("http://DASHCAM32_V2.local/");
    Serial.flush();
...
void ConnecToWiFi(){
  ...
  MyDns();
  Serial.write(3);  // Invia il comando '3' per indicare "Wi-Fi connesso"
  Serial.flush(); 
  Serial.println("\n connesso! \n");
...
String processor(const String & par){//mi da indicazione sull' attività o l'inattività della fotocamera dell'esp32_cam
  ...
  }else if (par =="AccX") //mi da indicazioni sull accelerazione in X dell'esp32_cam
  {
    if (flag != 1)
    {
      sprintf(SaccelX,"%d",accelX);
      return SaccelX;
    }
    else{
      return "Errore";
    }
  } else if (par =="AccY") //mi da indicazioni sull accelerazione in Y dell'esp32_cam
  {
    if (flag != 1)
    {
      sprintf(SaccelY,"%d",accelY);
      return SaccelY;
    }
    else{
      return "Errore";
  ...
  }else  if (par =="AccZ") //mi da indicazioni sull accelerazione in Z dell'esp32_cam
  {
    if (flag != 1)
    {
      sprintf(SaccelZ,"%d",accelZ);
      return SaccelZ;
    }
    else{
      return "Errore";
...
void setup() {
    ...
    server.on("/accelereometer", HTTP_GET, [](AsyncWebServerRequest *req){//pagina che visualizza le accelerazioni
